...
## The decisive separating experiment — RUN, decoder-bound (2026-07-09)

The committee's decisive separating test was chosen over the staircase and run (`reveal-the-set-cell.md`, `RunRevealSet`): reveal the 8-word target set to the society (collapse the open search to the true 3-bit job) at p = 0.7 vs the withheld baseline. **Result: revealing the set unblocks — B = 5/62 vs A = 0/64, Newcombe B − A = [0.008, 0.175] (excludes 0).** So the p = 0.7 null is **decoder-bound**: the open search was the binding constraint the tier ladder could not touch, and the channel permits winning the collapsed task. This confirms the decomposition empirically — the channel term is tier-invariant (a stronger tier nulls), and the decoder's open search (a protocol property) is the constraint that revealing the set removes. See the reveal-the-set section in `fallible-oracle-results.md`.
