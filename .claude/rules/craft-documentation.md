...
> See `craft-complexity.md` for the obviousness-and-naming side of this discipline and the design budget that funds it (this rule is the canonical home for comments), `craft-abstraction.md` for the specification-vs-implementation split a contract comment captures, `craft-tdd.md` for the comment-first/test-first parallel, and the active language overlay (`scala-style.md`) for the comment and docstring mechanics — syntax, summary mood, and the Args/Returns/Raises/Errors/Panics/Safety sections this rule deliberately leaves to them.
