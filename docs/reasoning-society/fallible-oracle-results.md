...
**N = 62 accounting, and the thin margin.** B ran the same 64 games as A (8 targets × 8 seeds); two — `cup` and `tree`, one seed each — errored mid-run against the live API and produced no completed record, so B is scored over the 62 that finished (A completed all 64). The asymmetry favors B only trivially: counting the two dropped games as non-wins (5/64 = 0.078 against A's 0/64) barely moves the point estimate. The effect is nonetheless thin — five wins, and the B − A lower bound sits at 0.008, a hair above zero — so the finding is **directional**: revealing the set moves the win rate off an *exact* zero (A is 0/64, not a rounded-down small number), which is the claim, but 0.08 is not a precise rate and a larger N would be needed to pin the magnitude. What the small N cannot erase is the sign, because the baseline it is measured against is a hard zero.
...
- **The reveal cell is a thin, directional result with a small denominator asymmetry.** B is scored over 62 games (`cup` and `tree`, one seed each, errored against the live API and were excluded); A over 64. The unblock rests on five wins with a B − A lower bound of 0.008, so it establishes the *sign* — revealing the set moves the win rate off an exact-zero baseline — but not a precise magnitude; the conservative recount (5/64) barely moves the point estimate, and a frontier-p estimate would need N ≈ 150–200. The reveal is delivered as a trusted system-prompt preamble naming the eight candidates — a strong intervention — so the cell isolates "decoder-bound versus channel-bound," not the finer question of how much of the search collapse is recognition versus reframing; the unchanged wrong-guess-reach rate (`w` = 0.016 in both arms) argues it collapses the search rather than inducing more guessing.
