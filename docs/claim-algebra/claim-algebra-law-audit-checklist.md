...
> **RECONCILED 2026-07-22 to the formal-edition revision of `claim-algebra.html` /
> `claim-calculus.html` — the revised HTML is authoritative on any disagreement.** The revision
> renamed, renumbered, and re-settled several results; the body below is updated to match, and the
> dated 2026-07-01/02 notes are retained as history (their now-reversed claims carry a pointer here):
> - **Supersession now uses `strike`, not `refute` (¬)** — `supersede(t, g) = (strike t, g)` (calculus
>   §1; algebra §5.6). This REVERSES the 2026-07-01/02 "it is refute, not strike" note below: `refute`
>   is involutive, so re-superseding an already-struck prior would swap its con back to pro and
>   resurrect the value; `strike` is idempotent and stays struck. `refute` remains the involutive
>   negation (§4.5); `strike` is now BOTH whole-slot withdrawal and the supersession retirement.
>   Pinned in `TestimonySuite` + `SupersessionStrikeSuite`.
> - **Renames:** "confluence" → **assertion permutation invariance** (Thm 5.1); "strong normalization"
>   → **finite-fold termination** (Thm 4.2); "withdrawal absorption" → **withdrawal idempotence** (Thm
>   6.1).
> - **Renumbering** (§6 reorganized): non-fabrication **6.2 → 6.3**; exclusivity **6.3 → 6.4**;
>   fail-closed propagation **6.7 → 6.5**; rehabilitation asymmetry → **Remark 6.2**; the full-language
>   permutation non-theorem → **Non-theorem 5.2**. The fine-grained draft identifiers the old body
>   cited (Def 2.14, Def 3.4, Prop 2.13, Def 2.16, Remark 5.4/6.4) were dropped — the §3 transition
>   table and §1 preliminaries carry that content now.
>
> The shipped `Ledger` fold and every coded law are unchanged and correct against the revision (a deep-
> examination committee confirmed it); this was a documentation reconciliation only.
>
> **Tracked in-repo, audited 2026-07-01.** The suite was audited against this checklist and the gaps
> closed: the calculus meta-theorems (non-fabrication, determinacy/termination, permutation invariance
> — see the renumbering above) in `LedgerLawsSuite`; the `Testimony`-generator distribution in
...
> Coverage is complete as of that date. _[The 2026-07-01 audit's supersession correction — "it is
> `refute`, not `strike`" — was itself reversed by the 2026-07-22 revision; see the top note.]_
>
...
> _[Both since reversed back to `strike` by the 2026-07-22 revision — see the top note.]_
...
- ✅ **Associativity + commutativity of `⊗ₖ, ⊕ₖ, ∧ₜ, ∨ₜ`.** §4.5. (Doc note: "lost only for sequential value-dependent derivation" — see Tier 3 assertion permutation invariance.)
...
- ✅ **Gap ≠ glut.** §4.5 `N ≠ B`, the two poles of `≤ₖ`. Structurally guaranteed by the independent pair `Ev(pro, con)` + `corner` reading both channels. Operational test: no combination identifies `N` and `B`; a property that catches "`con := ⊤ ⊖ pro`" collapse. ➕ **The corner is a presence read, never a magnitude read** (calculus Def 2.6 — model-free, `provPro.isZero`/`provCon.isZero`): any nonzero con blocks `True` however small (Thm 6.4, exclusivity), and a weakly supported unrefuted value is still `True`. Do not restate the corners as high/low quadrants (the 2026-07-02 validation caught exactly this gloss in the activation brief).
...
- ✅ ✂️ ➕ **Withdrawal `strike` — idempotent (DISTINCT from ¬).** `claim-calculus` Theorem 6.1 (withdrawal idempotence): `strike(strike t) = strike t`. The doc is explicit that deletion is modeled as `strike`, **not** `¬`, precisely because "a second refute would swap the channels back and resurrect the value." `refute` and `strike` are two different operations with two different laws; test both, and do **not** test `strike` for involution or `refute` for idempotence. (Under the 2026-07-22 revision `strike` is ALSO the supersession retirement — see below — for the same idempotence reason.)
- ✅ ➕ **Token withdrawal `withoutToken` (∖) — the token-scoped deletion.** `claim-calculus` §2 syntax / the §3 `WithdrawToken` transition: `t ∖ l` drops one assertion's support (the token `l`) from both channels, additive and idempotent, leaving **no** con-residue. The completion of the deletion op-set beside whole-slot `strike`: strike **moves** support to con (so a re-assertion gluts forever), `∖` **removes** it (so a re-assertion signs) — the L11 divergence, and the reason it is a distinct operation, not a special case of strike. Test idempotence (L7), quarantine (L8 — the token absent from both channel supports), no-resurrection (L9), disjoint-corroborate commutation (L10), and the drop-vs-move divergence (L11). `FoldSemanticsSuite` L7–L11 + the `Prov.withoutToken` unit; `Ledger.Evidence.WithdrawnToken` folds it. Well-formedness: token-uniqueness (a lineage id names exactly one assertion), the identity-analog of the single-writer discipline (the operational counterpart of Non-theorem 5.2) — audit-fidelity, not gate-safety (a collision over-drops, fail-closed).
- ✅ ✂️ **Supersession = `strike`(old, kept on record) + fresh `T`.** Calculus §1 / the §3 Repl+Supersede transition: `supersede(t, g) = (strike t, g)` — modeled as *two* claims, never one, the prior struck onto the con-channel and kept, the amendment governing. It is `strike`, **not** `refute` (¬): the two coincide on a clean prior but diverge on a contested one — strike clears pro and folds all prior evidence to con, refute swaps the channels. `strike` is chosen because it is idempotent — re-superseding an already-struck prior keeps it struck, whereas `refute` (involutive) would swap the con back to pro and resurrect the value (algebra §5.6, "negating an already-refuted prior would incorrectly convert its con evidence into pro evidence"). Collapsing the pair into one testimony would put the prior's support alongside the operative's and misread it as a glut. Test the pairing, the trace retention, and the strike-vs-refute choice on a contested prior (`TestimonySuite` + `SupersessionStrikeSuite`); the operative-signs-clean property (con empty, nothing of the prior leaking in) across every public fold path is `FoldSemanticsSuite` L1/L2.
...
- ✅ **Finite-fold termination (Thm 4.2).** Evaluation halts in exactly `|E|` steps. ➕
- ✅ **Assertion permutation invariance (Thm 5.1).** For an assertion term, `belief(E)` depends only on the *multiset* of testimonies, not their order (the commutative-fragment result).
- ✅ **Withdrawal idempotence (Thm 6.1).** (See `strike`, above.) Belief-level idempotence on both shapes: `Open` in `LedgerFoldSuite`, `Repl` (the supersession pair) in `FoldSemanticsSuite` (L5).
- ✅ ➕ **Rehabilitation asymmetry (Remark 6.2).** A withdrawn value returns only by *supersession*, never by *re-assertion*: after a whole-slot `Withdraw`, a fresh `Assert` of the same value gluts forever (ℕ[X] is zero-sum-free, so con never cancels), while a `Supersede` signs. Pinned in `FoldSemanticsSuite` (L3 gluts-forever, L4 supersede-signs).
- ✅ ➕ **Chained supersession keeps depth one (the §3 Repl+Supersede transition).** Re-superseding discards the earlier struck prior from the normal form (keeps only `strike(operative(s))`); the full audit lives in the event term, recovered by re-fold. Pinned in `FoldSemanticsSuite` (L6). Matches `Ledger` and the §3 table — a clarification, not a correction.
- ✅ **Non-fabrication (Thm 6.3).** If a term signs `v`, then `v` originates in `E`. ➕ Pinned generatively at the fold in `LedgerLawsSuite`.
- ✅ **Exclusivity (Thm 6.4).** A term never signs a value carrying any con-support; a glutted value is never signed. ➕ (This, plus 6.3 and 6.5, is what "subject reduction / safety" actually *is* in your §6 — replace the generic "reduction preserves well-formedness" I had.) Pinned generatively at the fold in `LedgerLawsSuite`.
- ✅ **Fail-closed propagation / no gap-laundering (Thm 6.5).** If any leaf of a derivation is a gap, `corner(⟦D⟧)=N` at the root and the gate blocks it. Uses only `⊗ₖ`. *The spine — matched verbatim.*
...
- ✅ Supporting: corroboration `⊞` is a commutative monoid (proven in the core law suites); assertion permutation invariance of the fold (Thm 5.1).
...
- **Full-language permutation invariance is a non-theorem (Non-theorem 5.2).** Permutation invariance holds *only* on the commutative (assertion) fragment (Thm 5.1). The supersession/withdrawal fragment is **order-sensitive by design** and must be **serialized per slot (single-writer discipline)** — that requirement is the operational counterpart of Non-theorem 5.2. So test order-independence on assertions; test *single-writer serialization* on supersession, and pin the order-sensitivity witness (`FoldSemanticsSuite` L12), not permutation invariance.
...
- ⚠️ **The multi-document corpus / version-precedence supersession** framing. The docs model supersession as `strike`(old, kept on record) `+` fresh `T`, **not** as version/precedence ranking over a document set. *Adopted* as the thin Scenario-A layer (N1–N4): precedence rank is computed by a pure policy at decision time, never stored, and a winner must strictly dominate — any tie or incomparability keeps the target a conflict.
...
separate idempotent operation from involutive `¬` (and, per the 2026-07-22 revision, is what
supersession now uses — see the top note), (3) `verify` is a `Bool`+opaque-`Claim`
