...
/** Pure, in-module unit coverage of the four-state read — [[BelnapReader.resolve]] /
  ...
  * grounding-coupled integration paths still live in the workbench's suites.
  *
  * Asserts the STRUCTURAL four-state contract — status, governing value, struck prior. The operator
  * `note` prose is presentation, deliberately NOT pinned here (it moves to a workbench presentation
  * layer in the note-residue refinement).
  */
class ResolveSuite extends munit.FunSuite:
  ...
  private def struct[A](r: Row[A]): (Status, Option[A], Option[A]) = (r.status, r.value, r.struck)

  test("resolve: a single grounded leaf signs — Resolved with that value") {
    assertEquals(
      struct(BelnapReader.resolve("debt", leaf("$100"))),
  ...
  test("resolve: the gap is Missing — fail-closed, nothing signs") {
    assertEquals(
      struct(BelnapReader.resolve("debt", Testimony.gap[String])),
  ...
  test("resolve: two rival for-values are Conflict — ambiguity, cardinality 2, no con-mass") {
    ...
    assertEquals(struct(BelnapReader.resolve("debt", ambiguous)), (Status.Conflict, None, None))
  }
  ...
  test("resolve: a value asserted AND refuted is Conflict — a genuine glut") {
    ...
    assertEquals(struct(BelnapReader.resolve("debt", glut)), (Status.Conflict, None, None))
  }

  test("resolve: a refuted leaf with no replacement is Superseded — the struck value retained") {
    assertEquals(
      struct(BelnapReader.resolve("debt", Testimony.refute(leaf("$100")))),
  ...
  test("resolveSuperseded: a clean amendment governs, the prior struck and retained") {
    ...
    assertEquals(
      struct(BelnapReader.resolveSuperseded("margin", s)),
  ...
  test("resolveAmended: an amendment silent on the item leaves the base standing — Resolved") {
    assertEquals(
      struct(BelnapReader.resolveAmended("debt", leaf("$100"), Testimony.gap[String])),
      (Status.Resolved, Some("$100"), None)
    )
  }
  ...
  test("resolveAmended: a base silent on the item takes the amendment as a fresh value") {
    assertEquals(
      struct(BelnapReader.resolveAmended("debt", Testimony.gap[String], leaf("$200"))),
  ...
  test("resolveAmended: both speak — the amendment supersedes and the prior is retained") {
    assertEquals(
      struct(BelnapReader.resolveAmended("debt", leaf("$100"), leaf("$200"))),
      (Status.Superseded, Some("$200"), Some("$100"))
    )
  }
